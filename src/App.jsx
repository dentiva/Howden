...
const buildPortfolioBrief = (portfolio, brokerName) => {
  ...
  return portfolio.map(c => ({
    ...
    recent_signals: (c.signals || []).map(s => s.title),
...
If the broker asks about something that is not present in this brief, say so directly and explain what information would be needed, rather than refusing to answer or asking the broker to open a client tile.

Style guidance:
Keep replies concise and business appropriate. Write in full sentences, not in fragments. Use Indian currency conventions such as 3.85 Crore Rupees or 21.4 Lakh Rupees. Avoid marketing language and hyphenated phrases such as "purpose built" or "AI powered". Do not invent numbers that are not listed in the brief. Format is plain prose with light markdown only where it aids comprehension.`;
  }
  const ctx = {
    client: client.name,
        ...
        <div className="flex flex-wrap items-start gap-6 justify-between">
          ...
          <div className="flex items-center gap-6 flex-wrap">
            <div className="flex flex-col items-center">
              ...
              <div className="text-[11px] mt-1" style={{ color: scoreTone(client.score).fg }}>{scoreTone(client.score).label}</div>
            </div>
            <div className="flex flex-col items-center">
              ...
              <div className="text-[11px] mt-1" style={{ color: ftone.fg }}>{ftone.label}</div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        <div className="lg:col-span-3 flex flex-col gap-4">
          ...
          <Panel
            ...
            subtitle="These are the features that raised or lowered the score."
            icon={BarChart3}
            headerRight={<AiTag />}
          >
            <ContribChart features={client.features} positiveLabel="Raises the score" negativeLabel="Lowers the score" />
          </Panel>

          <Panel
            title="How hard this will be"
          ...
          >
            <ContribChart
              ...
              positiveLabel="Makes the sale harder"
              negativeLabel="Makes the sale easier"
            />
          </Panel>
...
const starters = (client) => {
  ...
  list.push(`Why did the opportunity score land at ${client.score}? Explain the top three drivers.`);
  return list;
};
    ...
    >
      ...
      <div className="p-3 hairline-t" style={{ background: 'var(--surface)' }}>
        ...
        <div className="text-[10px] mt-1.5 flex items-center gap-1" style={{ color: 'var(--ink-4)' }}>
          <Sparkles size={9} /> Claude Sonnet 4.5. The reply is grounded in the client data shown on this page.
        </div>
      </div>
    </aside>
  );
